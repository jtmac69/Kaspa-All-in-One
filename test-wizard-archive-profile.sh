...
# Kaspa All-in-One - Archive Profile End-to-End Test
# Tests the complete wizard flow with Archive profile installation
# Part of Task 2.1: Test all profiles - Archive profile
...
cleanup() {
    if [[ "$CLEANUP_ON_EXIT" == "true" ]]; then
        ...
        docker compose --profile archive down 2>/dev/null || true
...
header "Archive Profile End-to-End Test"
info "Testing wizard installation with Archive profile"
info "Profile: $PROFILE"
info "Services: archive-db, archive-indexer"
...
test_profiles_api() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # Check if archive profile exists
        local archive_exists=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="archive") | .id' 2>/dev/null || echo "")
        
        if [ "$archive_exists" = "archive" ]; then
            pass "Archive profile is available"
            
            # Display archive profile details
            local archive_name=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="archive") | .name' 2>/dev/null)
            local archive_services=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="archive") | .services | join(", ")' 2>/dev/null)
            local archive_deps=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="archive") | .dependencies | join(", ")' 2>/dev/null)
            info "Archive profile: $archive_name"
            info "Services: $archive_services"
            info "Dependencies: $archive_deps"
            
            return 0
        else
            fail "Archive profile not found in API response"
            return 1
        fi
    else
...
test_config_generation() {
    ...
    log "Generating configuration for Archive profile..."
  ...
  "profiles": ["archive"]
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -n "$env_content" ] && [ "$env_content" != "null" ]; then
            ...
            # Verify archive profile services are in config
            if echo "$env_content" | grep -q "TIMESCALEDB"; then
                verbose "✓ Archive Database configuration found"
            fi
...
test_installation() {
    ...
    log "Starting installation of Archive profile..."
  ...
  "profiles": ["archive"]
    ...
    test_api "GET" "/api/install/status/kaspa-archive-node" 200 "" || true

    # Deploy services
    log "Deploying services..."
    local deploy_result=$(test_api "POST" "/api/install/deploy" 200 '{"profiles":["archive"]}')
    ...
    while [ $elapsed -lt $TEST_TIMEOUT ]; do
        ...
        # Check if archive-db container is running
        if sudo docker ps --format '{{.Names}}' | grep -q "archive-db"; then
...
test_service_validation() {
    ...
    local validation_result=$(test_api "POST" "/api/install/validate" 200 '{"profiles":["archive"]}')
...
# Test 9: Archive services running
test_archive_services() {
    header "Test 9: Archive Services Running"
    TESTS_RUN=$((TESTS_RUN + 1))
    
    log "Checking Archive profile services..."
    
    local archive_services=("archive-db" "archive-indexer")
    local running=0
    local total=${#archive_services[@]}
    ...
    if [ $running -eq $total ]; then
        pass "All Archive services are running ($running/$total)"
        return 0
    elif [ $running -gt 0 ]; then
        warn "Some Archive services are running ($running/$total)"
        return 0
    else
        fail "No Archive services are running ($running/$total)"
        return 1
    fi
}
...
# Test 11: Archive Database accessibility
test_archive-db_access() {
    header "Test 11: Archive Database Accessibility"
    TESTS_RUN=$((TESTS_RUN + 1))
    
    log "Checking Archive Database accessibility..."
    
    # Wait a bit for services to fully start
    sleep 10
    
    # Check if Archive Database port is accessible
    if nc -z localhost 5433 2>/dev/null; then
        pass "Archive Database is accessible on port 5433"
        return 0
    else
        warn "Archive Database port 5433 is not yet accessible (may still be starting)"
        return 0
    fi
}
...
main() {
    ...
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}║       Kaspa All-in-One - Archive Profile E2E Test           ║${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    test_archive_services || exit 1
    test_core_dependency || exit 1
    test_archive-db_access || exit 1
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "${GREEN}Archive profile installation completed successfully!${NC}"
        echo ""
        echo -e "${BLUE}Next Steps:${NC}"
        echo "  1. Access dashboard: http://localhost:8080"
        echo "  2. Connect to Archive Database: psql -h localhost -U kaspa -d simply_kaspa"
        echo "  3. Check service logs: docker compose logs -f"
        echo "  4. Stop services: docker compose --profile archive down"
        echo ""
        exit 0
    else
        echo -e "${RED}╔═══════════════════════════════════════════════════════════════╗${NC}"
        ...
        echo "  4. Check Archive Database: docker compose logs archive-db"
        echo ""
        exit 1
    fi
}
