...
# Kaspa All-in-One - Explorer Profile End-to-End Test
# Tests the complete wizard flow with Explorer profile installation
...
TEST_TIMEOUT=600  # 10 minutes for installation (larger profile)
...
test_profiles_api() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # Check if explorer profile exists (handle both array and object responses)
        local explorer_exists=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="explorer") | .id' 2>/dev/null || echo "")
        
        if [ "$explorer_exists" = "explorer" ]; then
            pass "Explorer profile is available"
            
            # Display explorer profile details
            local explorer_name=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="explorer") | .name' 2>/dev/null)
            local explorer_services=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="explorer") | .services | join(", ")' 2>/dev/null)
            local explorer_deps=$(echo "$result" | jq -r '.[] | select(.id=="explorer") | .dependencies | join(", ")' 2>/dev/null)
            info "Explorer profile: $explorer_name"
            info "Services: $explorer_services"
            info "Dependencies: $explorer_deps"
            
            return 0
        else
            fail "Explorer profile not found in API response"
            return 1
        fi
    else
...
test_config_generation() {
    ...
    log "Generating configuration for Explorer profile..."
  ...
  "profiles": ["explorer"]
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -n "$env_content" ] && [ "$env_content" != "null" ]; then
            ...
            # Verify explorer profile services are in config
...
test_installation() {
    ...
    log "Starting installation of Explorer profile..."
  ...
  "profiles": ["explorer"]
    ...
    test_api "GET" "/api/install/status/kaspa-explorer" 200 "" || true

    # Deploy services
    log "Deploying services..."
    local deploy_result=$(test_api "POST" "/api/install/deploy" 200 '{"profiles":["explorer"]}')
...
test_service_validation() {
    ...
    local validation_result=$(test_api "POST" "/api/install/validate" 200 '{"profiles":["explorer"]}')
...
# Test 9: Explorer services running
test_explorer_services() {
    header "Test 9: Explorer Services Running"
    TESTS_RUN=$((TESTS_RUN + 1))
    ...
    if [ $running -eq $total ]; then
        pass "All Explorer services are running ($running/$total)"
        return 0
    elif [ $running -gt 0 ]; then
        warn "Some Explorer services are running ($running/$total)"
        return 0
    else
        fail "No Explorer services are running ($running/$total)"
        return 1
    fi
}
...
main() {
    ...
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}║       Kaspa All-in-One - Explorer Profile E2E Test           ║${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    test_explorer_services || exit 1
    test_core_dependency || exit 1
    test_timescaledb_access || exit 1
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "${GREEN}Explorer profile installation completed successfully!${NC}"
        echo ""
