...
# Kaspa All-in-One - Development Profile End-to-End Test
# Tests the complete wizard flow with Development profile installation
# Part of Task 2.6: Test all profiles - Development profile
...
header "Development Profile End-to-End Test"
info "Testing wizard installation with Development profile"
info "Profile: $PROFILE"
info "Services: portainer, pgadmin"
...
test_profiles_api() {
    ...
    if [ $? -eq 0 ]; then
        ...
        local dev_exists=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="development") | .id' 2>/dev/null || echo "")
        
        if [ "$dev_exists" = "development" ]; then
            pass "Development profile is available"
            
            # Display development profile details
            local dev_name=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="development") | .name' 2>/dev/null)
            local dev_services=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="development") | .services | join(", ")' 2>/dev/null)
            local dev_deps=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="development") | .dependencies | join(", ")' 2>/dev/null)
            info "Development profile: $dev_name"
            info "Services: $dev_services"
            info "Dependencies: $dev_deps"
            
            return 0
        else
            fail "Development profile not found in API response"
            return 1
        fi
    else
...
test_config_generation() {
    ...
    log "Generating configuration for Development profile..."
   ...
   false,
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -n "$env_content" ] && [ "$env_content" != "null" ]; then
            ...
            # Verify development profile services are in config
            if echo "$env_content" | grep -q "PORTAINER"; then
                verbose "✓ Portainer configuration found"
            fi
            
            if echo "$env_content" | grep -q "PGADMIN"; then
                verbose "✓ pgAdmin configuration found"
            fi
...
test_installation() {
    ...
    log "Starting installation of Development profile..."
  ...
  "profiles": ["development"]
    ...
    local deploy_result=$(test_api "POST" "/api/install/deploy" 200 '{"profiles":["development"]}')
...
test_service_validation() {
    ...
    local validation_result=$(test_api "POST" "/api/install/validate" 200 '{"profiles":["development"]}')
...
# Test 9: Development services running
test_development_services() {
    header "Test 9: Development Services Running"
    TESTS_RUN=$((TESTS_RUN + 1))
    
    log "Checking Development profile services..."
    
    local dev_services=("portainer" "pgadmin")
    local running=0
    local total=${#dev_services[@]}
    ...
    if [ $running -eq $total ]; then
        pass "All Development services are running ($running/$total)"
        return 0
    elif [ $running -gt 0 ]; then
        warn "Some Development services are running ($running/$total)"
        return 0
    else
        fail "No Development services are running ($running/$total)"
        return 1
    fi
}
...
# Test 11: Development tools accessibility
test_dev_tools_access() {
    header "Test 11: Development Tools Accessibility"
    TESTS_RUN=$((TESTS_RUN + 1))
    
    log "Checking development tools accessibility..."
    ...
    # Check Portainer (port 9000)
    checks=$((checks + 1))
    if nc -z localhost 9000 2>/dev/null; then
        verbose "✓ Portainer is accessible on port 9000"
        passed=$((passed + 1))
    else
        verbose "✗ Portainer port 9000 is not accessible"
    fi
    
    # Check pgAdmin (port 5050)
    checks=$((checks + 1))
    if nc -z localhost 5050 2>/dev/null; then
        verbose "✓ pgAdmin is accessible on port 5050"
        passed=$((passed + 1))
    else
        verbose "✗ pgAdmin port 5050 is not accessible"
    fi
    
    if [ $passed -eq $checks ]; then
        pass "All development tools are accessible ($passed/$checks)"
        return 0
    elif [ $passed -gt 0 ]; then
        warn "Some development tools are accessible ($passed/$checks)"
        return 0
    else
        warn "Development tools are not yet accessible (may still be starting)"
        return 0
    fi
}
...
main() {
    ...
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}║     Kaspa All-in-One - Development Profile E2E Test          ║${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    test_development_services || exit 1
    test_core_dependency || exit 1
    test_dev_tools_access || exit 1
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "${GREEN}Development profile installation completed successfully!${NC}"
        echo ""
        echo -e "${BLUE}Next Steps:${NC}"
        echo "  1. Access dashboard: http://localhost:8080"
        echo "  2. Access Portainer: http://localhost:9000"
        echo "  3. Access pgAdmin: http://localhost:5050"
        echo "  4. Check service logs: docker compose logs -f"
        echo "  5. Stop services: docker compose --profile development down"
        echo ""
        exit 0
    else
        echo -e "${RED}╔═══════════════════════════════════════════════════════════════╗${NC}"
        ...
        echo "  4. Check Portainer logs: docker compose logs portainer"
        echo "  5. Check pgAdmin logs: docker compose logs pgadmin"
        echo ""
        exit 1
    fi
}
