...
# Kaspa All-in-One - Core Profile End-to-End Test
# Tests the complete wizard flow with Core profile installation
...
header "Core Profile End-to-End Test"
info "Testing wizard installation with Core profile"
info "Profile: $PROFILE"
info "Services: kaspa-node, dashboard, nginx"
echo ""
...
test_profiles_api() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # Check if core profile exists (handle both array and object responses)
        local core_exists=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="core") | .id' 2>/dev/null || echo "")
        
        if [ "$core_exists" = "core" ]; then
            pass "Core profile is available"
            
            # Display core profile details
            local core_name=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="core") | .name' 2>/dev/null)
            local core_services=$(echo "$result" | jq -r '.profiles[]? // .[] | select(.id=="core") | .services | join(", ")' 2>/dev/null)
            info "Core profile: $core_name"
            info "Services: $core_services"
            
            return 0
        else
            fail "Core profile not found in API response"
            return 1
        fi
    else
...
test_config_generation() {
    ...
    log "Generating configuration for Core profile..."
  ...
  "profiles": ["core"]
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -n "$env_content" ] && [ "$env_content" != "null" ]; then
            ...
            # Verify core profile services are in config
            if echo "$env_content" | grep -q "KASPA_NODE"; then
                verbose "✓ Kaspa node configuration found"
            fi
...
test_installation() {
    ...
    log "Starting installation of Core profile..."
  ...
  "profiles": ["core"]
    ...
    local deploy_result=$(test_api "POST" "/api/install/deploy" 200 '{"profiles":["core"]}')
    ...
    while [ $elapsed -lt $TEST_TIMEOUT ]; do
        ...
        if sudo docker ps --format '{{.Names}}' | grep -q "kaspa-node"; then
...
test_service_validation() {
    ...
    local validation_result=$(test_api "POST" "/api/install/validate" 200 '{"profiles":["core"]}')
...
# Test 9: Core services running
test_core_services() {
    header "Test 9: Core Services Running"
    TESTS_RUN=$((TESTS_RUN + 1))
    
    log "Checking Core profile services..."
    ...
    if [ $running -eq $total ]; then
        pass "All Core services are running ($running/$total)"
        return 0
    elif [ $running -gt 0 ]; then
        warn "Some Core services are running ($running/$total)"
        return 0
    else
        fail "No Core services are running ($running/$total)"
        return 1
    fi
}
...
test_dashboard_access() {
    header "Test 10: Dashboard Accessibility"
    TESTS_RUN=$((TESTS_RUN + 1))
    ...
    if wait_for_service "http://localhost:8080" "Dashboard" 60; then
        pass "Dashboard is accessible at http://localhost:8080"
        return 0
    else
        warn "Dashboard is not yet accessible (may still be starting)"
        return 0
    fi
}
...
main() {
    ...
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}║         Kaspa All-in-One - Core Profile E2E Test             ║${NC}"
    echo -e "${CYAN}║                                                               ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    test_core_services || exit 1
    test_dashboard_access || exit 1
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "${GREEN}Core profile installation completed successfully!${NC}"
        echo ""
        echo -e "${BLUE}Next Steps:${NC}"
        echo "  1. Access dashboard: http://localhost:8080"
        echo "  2. Check service logs: docker compose logs -f"
        echo "  3. Stop services: docker compose --profile core down"
        echo ""
        exit 0
    else
        echo -e "${RED}╔═══════════════════════════════════════════════════════════════╗${NC}"
