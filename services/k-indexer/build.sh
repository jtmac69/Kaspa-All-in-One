...
Build modes:
  docker          Build with current K_INDEXER_VERSION (default: master)
...
Examples:
  ...
  $0 version v2.1.0          # Build specific version
  $0 timescaledb             # Build with TimescaleDB optimizations
  $0 personal k-indexer-personal  # Build with Personal Indexer features
  $0 prod k-indexer-prod     # Production build with custom tag
...
build_image() {
    ...
    log_info "Building K-Indexer with version: $version"
    ...
    local build_args="--build-arg K_INDEXER_VERSION=$version"
    ...
    build_args="$build_args --label k-indexer.version=$version"
    build_args="$build_args --label k-indexer.timescaledb=$enable_timescaledb"
    build_args="$build_args --label k-indexer.personal-mode=$personal_mode"
    build_args="$build_args --label k-indexer.build-date=$(date -u +'%Y-%m-%dT%H:%M:%SZ')"
...
run_performance_test() {
    ...
    # This would run a simple performance test
    # For now, just check image size and startup time
    local image_size
...
main() {
    ...
    if [[ "$MODE" == "timescaledb" || "$MODE" == "personal" || "$MODE" == "dev" || "$MODE" == "prod" ]]; then
    ...
    log_info "K-Indexer build process completed successfully!"
